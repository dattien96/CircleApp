...
//            replaceFragment(PlayFragment(), PlayFragment.TAG, true)
        ...
        btn_frame.setOnClickListener {
            val tmp = startTime
            startTime += 500
            var root = GameCoordinate(
                tmp,
                startTime,
                mutableListOf<Item>().apply { addAll(items) })
            items.clear()
            AppMaterialIdLocator.root.add(root)
        }
        btn_santa.setOnClickListener {
            var type = ItemJsonType.SANTA
            var item =
                Item(type, pointA!!, pointB!!)
            items.add(item)
        }
        btn_rein.setOnClickListener {
            var type = ItemJsonType.REINDEER
            var item =
                Item(type, pointA!!, pointB!!)
            items.add(item)
        }
        btn_snow.setOnClickListener {
            var type = ItemJsonType.SNOW_MAN
            var item =
                Item(type, pointA!!, pointB!!)
            items.add(item)
        }
        btn_bird.setOnClickListener {
            var type = ItemJsonType.BIRD
            var item =
                Item(type, pointA!!, pointB!!)
            items.add(item)
        }
        btn_plane.setOnClickListener {
            var type = ItemJsonType.PLANE
            var item =
                Item(type, pointA!!, pointB!!)
            items.add(item)
        }
        ...
        btn_log.setOnClickListener {
            Log.d("asdasd", "${AppMaterialIdLocator.root}")
        }

        btn_convert.setOnClickListener {
            val gson = Gson()
            val x = gson.toJson(AppMaterialIdLocator.root)
            Log.d("asdasd", "$x")
        }
    ...
    private fun initNormalPlayerList() {
        ...
        player = video_view?.initPlayer(this@StartFragment.context ?: return)
        ...
        val mediaSource =
            buildLoopMediaSource(this@StartFragment.context ?: return, uri)
        player?.apply {
            playWhenReady = true
