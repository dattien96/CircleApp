...
import com.datnht.circleapp.newsource.ItemJsonType
...
import kotlinx.android.synthetic.main.fragment_start_record.*

class RecordStartScreen : BaseFragment() {
    override val layoutId: Int
        get() = R.layout.fragment_start_record
...
//            replaceFragment(PlayFragment(), PlayFragment.TAG, true)
        ...
        btn_frame.setOnClickListener {
            var root = StartCoordinate(
                mutableListOf<Item>().apply { addAll(items) })
            items.clear()
            AppMaterialIdLocator.startRoot = root
        }
        btn_back.setOnClickListener {
            var type = ItemJsonType.START_BACK
            var item =
                Item(type, pointA!!, pointB!!)
            items.add(item)
        }
        btn_start.setOnClickListener {
            var type = ItemJsonType.START
            var item =
                Item(type, pointA!!, pointB!!)
            items.add(item)
        }
        btn_gift.setOnClickListener {
            var type = ItemJsonType.START_GIFT
            var item =
                Item(type, pointA!!, pointB!!)
            items.add(item)
        }
        ...
        btn_convert.setOnClickListener {
            val gson = Gson()
            val x = gson.toJson(AppMaterialIdLocator.startRoot)
            Log.d("asdasd", "$x")
        }
    ...
    private fun initNormalPlayerList() {
        ...
        player = video_view?.initPlayer(this@RecordStartScreen.context ?: return)
        val dataSpec = DataSpec(RawResourceDataSource.buildRawResourceUri(R.raw.intro))
        ...
        val mediaSource =
            buildLoopMediaSource(this@RecordStartScreen.context ?: return, uri)
        player?.apply {
            playWhenReady = true
